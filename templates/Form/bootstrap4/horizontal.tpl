<div class="form-group {$element->attrs()->id()}{if $element->hasError()} has-error{/if}">
    {block name='label'}
        {if $element->label()}
            <label for="{$element->attrs()->id()}" class="control-label col-sm-{$element->columnSize()}">
                ...
                {if $element->tooltip()}
                    <i class="fa fa-question-circle" data-toggle="tooltip" data-placement="auto top" data-container="body" title="{$element->tooltip()}"></i>
                {/if}
            </label>
        {/if}
    {/block}
    ...
    <div class="col-sm-{$column_size}">
        ...
        <div id="{$element->attrs()->id()}-error" class="nataphp-error-message">
            {if $element->hasError()}
                <p class="form-text help-block text-muted"><small><i class="fa fa-times"></i> {$element->hasError()}</small></p>
            {/if}
        </div>
        {if $element->description()}
            <p class="form-text help-block text-muted"><small><em>{$element->description()}</em></small></p>
        {/if}
    </div>
</div>