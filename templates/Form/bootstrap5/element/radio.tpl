    ...
    {foreach $element->options()->get() as $option}
        ...
        <div class="form-check{if $option->disabled()} opacity-50{/if}{if $element->inline()} form-check-inline{/if}">
            <label class="form-check-label{if $element->inline()} me-3{/if}">
                ...
                <input type="radio" class="form-check-input" id="{$option->id()}" name="{$element->attrs()->name()}" value="{$option->value()}"{if $option->disabled()} disabled="disabled"{/if}{if $option->checked()} checked="checked"{/if} {$option->attributes()->toHtml()} />
