 ...
 * - `sortWhitelist` - A list of sort fields/parameters users are allowed to set using request.
 *
 * @var array
 */
...
/**
 * Process the pagination data into array of elements
 *
 * @param \Nata\ORM\Query $object Parameters
 * @return array Pagination parameters
 */
    public function initialize(array $config) {
        $this->_controller->viewBuilder()->loadHelper('Paginator');
    }
 ...
 *
 * By default NataPHP will automatically allow sorting on any column on the table object being
 ...
 */
    public function paginate($object, array $settings = []) {
        ...
        if (empty($query)) {
            ...
            $order = $query->order();
            $paramManager = $query->getParameterManager();
            $params = [];
            if (!empty($order) && empty($options['order'])) {
                $options['order'] = $order;
                $params = $query->getParameterManager()->get('orderBy');
            }
            $query->applyOptions($options);
            $paramManager->add($params, 'orderBy');
        }
        ...
        $page = max(min($requestedPage, $pageCount), 1);
        ...
        $paging = [
            ...
            'count' => $count,
            'perPage' => $limit,
            'prevPage' => ($page > 1), // There's a previous page
            'nextPage' => ($count > ($page * $limit)), // There's a next page
            'pageCount' => $pageCount,
            'sort' => isset($order[0]) ? [] : $order,
 ...
 */
    public function mergeOptions($alias, $settings) {
        ...
        if (is_array($page) && isset($page[$alias])) {
 ...
 *
 * @param \Nata\ORM\Table $object Repository object.
 * @param array $options The pagination options being used for this request.
 * @return array An array of options with sort + direction removed and replaced with order if possible.
 */
    public function validateSort(Table $object, array $options) {
        if (isset($options['sortWhitelist']) && is_array($options['sortWhitelist']) && empty($options['sortWhitelist'])) {
            return $options;
        }
        ...
        $options['order'] = $this->_prefix($object, $options);
 ...
 */
    protected function _prefix(Table $object, array $options): array {
        $tableAlias = $object->alias();
        $tableOrder = [];
        $sortWhitelist = isset($options['sortWhitelist']) ? $options['sortWhitelist'] : [];
        ...
        foreach ($options['order'] as $key => $value) {
            ...
            $whitelisted = in_array($field, $sortWhitelist, true) || in_array($alias . '.' . $field, $sortWhitelist, true);
