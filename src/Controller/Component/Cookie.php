 ...
 * In addition to the basic features offered be \Nata\Http\Response, this class lets you:
 *
 ...
 * - `sameSite` - It takes three possible values: 'Strict', 'Lax', and 'None'.
 *       'Strict': the cookie is sent only to the same site as the one that originated it;
 *       'Lax': is similar, with an exception for when the user navigates to a URL from an external site, such as by following a link;
 *       'None': has no restrictions on cross-site requests.
...
/**
 * A reference to the Controller's Nata\Http\Response object
 *
 ...
 */
    ...
    protected function _load($key) {
        ...
        if (!$this->_request->cookies($first)) {
            return;
        }
