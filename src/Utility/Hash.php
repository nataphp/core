 ...
 *
 * @param \ArrayAccess|array $data Array of data to operate on.
 ...
 */
    public static function get($data, $path, $default = null) {
        ...
        if (!is_array($data) && !($data instanceof ArrayAccess)) {
            throw new InvalidArgumentException('Data passed to Hash::get() must be an array or instance of ArrayAccess');
        }
 ...
 */
    public static function extract($data, $path) {
        ...
        if (!is_array($data) && !($data instanceof ArrayAccess)) {
            throw new InvalidArgumentException('Data passed to Hash::extract() must be an array or instance of ArrayAccess');
        }
 ...
 *
 * @param \ArrayAccess|array $data The data to insert into.
 ...
 */
    public static function insert($data, $path, $values = null) {
        if (!is_array($data) && !($data instanceof ArrayAccess)) {
            throw new InvalidArgumentException('Data passed to Hash::insert() must be an array or instance of ArrayAccess');
        }

        $hasBrackets = strpos($path, '[') !== false;
        if (!$hasBrackets) {
            $tokens = explode('.', $path);
        } else {
            $tokens = Text::tokenize($path, '.', '[', ']');
        ...
        if (strpos($path, '{') === false && !$hasBrackets) {
 ...
 */
    public static function remove($data, $path) {
        if (!is_array($data) && !($data instanceof ArrayAccess)) {
            throw new InvalidArgumentException('Data passed to Hash::remove() must be an array or instance of ArrayAccess');
        }
        if (strpos($path, '[') === false) {
            $tokens = explode('.', $path);
        } else {
 ...
 *
 * @param \ArrayAccess|array $data Array from where to extract keys and values
 ...
 */
    public static function combine($data, $keyPath, $valuePath = null, $groupPath = null) {
        ...
        if (!is_array($data) && !($data instanceof ArrayAccess)) {
            throw new InvalidArgumentException('Data passed to Hash::combine() must be an array or instance of ArrayAccess');
        }
 ...
 *
 * @param \ArrayAccess|array $data Source array from which to extract the data
 ...
 */
    public static function format($data, array $paths, $format) {
        if (!is_array($data) && !($data instanceof ArrayAccess)) {
            throw new InvalidArgumentException('Data passed to Hash::format() must be an array or instance of ArrayAccess');
        }
        $extracted = array();
        $count = count($paths);
 ...
 *
 * @param \ArrayAccess|array $data The data to search through.
 ...
 */
    public static function contains($data, array $needle) {
        ...
        if (!is_array($data) && !($data instanceof ArrayAccess)) {
            throw new InvalidArgumentException('Data passed to Hash::contains() must be an array or instance of ArrayAccess');
        }
 ...
 *
 * @param \ArrayAccess|array $data The data to check.
 ...
 */
    public static function check($data, $path) {
        if (!is_array($data) && !($data instanceof ArrayAccess)) {
            throw new InvalidArgumentException('Data passed to Hash::check() must be an array or instance of ArrayAccess');
        }
...
/**
 * Collapses a multi-dimensional array into a single dimension with keys as values,
 * i.e. array(array('Foo' => array('Bar' => 'Far'))) becomes array(0, 'Foo', 'Bar').
 *
 * @param array $data Array to flatten
 * @return array
 */
    public static function flattenKeys(array $data) {
        $result = array();
        foreach ($data as $key => $v) {
            $result[] = $key;
            if (is_array($v)) {
                $result = array_merge($result, static::flattenKeys($v));
            }
        }
        return $result;
    }
    ...
    public static function merge(array $data, $merge) {
        ...
        while (($arg = next($args)) !== false) {
            foreach ((array)$arg as $key => $val) {
                ...
                } elseif (is_int($key) && isset($return[$key]) && !in_array($val, $return)) {
                    $return[] = $val;
                } else {
                    $return[$key] = $val;
                }
...
/**
 * Check if given needle it's in the given haystack.
 *
 * This differs from in_array because it accepts an array as needle.
 * If needle it's not an array, fallsback to in_array.
 *
 * @param array|string|int $needle Value to search or array of values
 * @param array $haystack List of values to check
 * @param boolean $all
 * @param boolean $strict
 * @return bool
 */
    public static function inArray($needle, array $haystack, bool $all = false, bool $strict = false): bool {
        if (!is_array($needle)) {
        ...
        if ($all !== false) {
            return count($needle) === count($result);
        }
        return true;
    }
...
/**
 * Permutation of given array values.
 *
 * @param array $elements Array elements to permutate
 * @return \Generator
 */
    public static function permutations(array $elements) {
        if (count($elements) <= 1) {
            ...
            foreach (static::permutations(array_slice($elements, 1)) as $permutation) {
