 ...
 *
 *
 * @param string $callback|Nata\Event\Event Method to fire on all the objects. Its assumed all the objects implement
 *   the method you are calling. If an instance of Nata\Event\Event is provided, then then Event name will parsed to
 ...
 * @throws Nata\Error\Exception when modParams is used with an index that does not exist.
 */
    public function trigger($callback, array $params = [], array $options = []) {
        ...
        if ($callback instanceof Event) {
            ...
            foreach (array_keys($defaultOptions) as $opt) {
                if ($optionValue = $event->data('objectCollectionOptions.' . $opt)) {
        ...
        if ($options['modParams'] !== false && !isset($params[$options['modParams']])) {
            throw new Exception('Cannot use modParams with indexes that do not exist.');
        }
