 ...
 */
    public static function paranoid($data, $allowed = []) {
        $allow = null;
        ...
        if (is_array($data)) {
            foreach ($data as $key => $val) {
                $data[$key] = static::paranoid($val, $allowed);
            }
            return $data;
        }

        return preg_replace("/[^[:alnum:][:space:]]/uig", '', $data);
    }
...
/**
 * Returns given data safe for display as HTML. Renders entities.
 *
 ...
 * @return string Sanitized string
 */
    public static function html($data, $options = []) {
        ...
        if (is_array($data)) {
            foreach ($data as $key => $val) {
                $data[$key] = static::html($val, $options);
            }
            return $data;
        }
        ...
        return htmlspecialchars($data, $options['quotes'], $options['charset'], $options['double']);
    }
 ...
 * @return string sanitized String
 */
    public static function stripTags(string $str, $tags) {
        if (!is_array($tags)) {
            $tags = func_get_args();
            unset($tags[0]);
        ...
        for ($i = 0, $count = count($tags); $i < $count; $i++) {
            $str = preg_replace('/<' . $tags[$i] . '\b[^>]*>/i', '', $str);
            $str = preg_replace('/<\/' . $tags[$i] . '[^>]*>/i', '', $str);
        }
 ...
 */
    public static function clean($data, $options = []) {
        ...
        if (is_array($data)) {
            foreach ($data as $key => $val) {
                $data[$key] = static::clean($val, $options);
            }
            return $data;
        }
