...
use Nata\Error\Debugger;
use Nata\Routing\Router;
use Nata\Error\Renderer;
use Throwable;
use Error;
use Exception;
 ...
 * `Renderer` is used.  Your custom exception renderer class should be placed in app/Lib/Error.
 *
 ...
 * Enabling this will log every exception to NataLog and the configured loggers.
 *
 * ### PHP errors
 *
 ...
 * errors will be logged to NataLog.  You can control which errors are logged by setting
 ...
 *
 * @see Renderer for more information on how to customize exception rendering.
 */
 ...
 * It will count number of thrown Error/Exception
 * and if more than X number of times, prevents from handling it.
 *
 * @var int
 */
    protected static $_errorCount = 0;
 ...
 */
    public static function handleException($exception) {
        if (static::_isRecursiveError()) {
            return null;
        }

        $config = Configure::read('Exception');

        $exceptionName = App::classShortName($exception);
        if (!empty($config['log']) && !in_array($exceptionName, (array)$config['skipLog'])) {
            $message = static::_getExceptionMessage($exception);
        ...
        try {
            ...
            set_error_handler(Configure::read('Error.handler')); // Should be using configured Error\Handler
            Configure::write('Error.trace', false); // trace is useless here since it's internal
            $message = static::_getExceptionMessage($t);
 ...
 */
    public static function handleError($code, $description, $file = null, $line = null, $context = null) {
        ...
        $message = static::_getRequestMessage($message);
        $message .= "\n";
 ...
 */
    public static function handleFatalError($code, $description, $file, $line) {
        ...
        $logMessage = static::_getRequestMessage($logMessage);
 ...
 */
    protected static function _getExceptionMessage($exception) {
 ...
 */
    protected static function _getRequestMessage($message = '') {
        $request = Router::getRequest();
        if ($request) {
            $message .= "[request] " . $request->clientIp() . " - " . $request->method() . " " . $request->host() . $request->here();
            if ($request->referer() !== '/') {
                $message .= ' < ' . $request->referer();
            }
            /*
            if ($request->data()) {
                $message .= PHP_EOL . json_encode($request->data());
            }
            */
        } elseif (env('CLI_MODE')) {
            $message .= "[cli] " . implode(' ', $_SERVER['argv']);
        }
        return $message;
    }

/**
 * Check if is a recursive error.
 *
 * @return bool True if it is, false otherwise
 */
    protected static function _isRecursiveError() {
        if (static::$_errorCount > 2) {
            return true;
        }
        static::$_errorCount++;
        return false;
