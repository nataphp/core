...
/**
 * Nata Cache CacheResource for Smarty.
 *
 * This implementation was extracted from Smarty_CacheResource_KeyValueStore
 * and applied to \Nata\Cache\Cache system.
 *
 * To avoid complications all cache-keys are translated to a sha1 hash.
 */
class Smarty_CacheResource_Natacache extends Smarty_CacheResource {
 ...
 */
    public function populate(Smarty_Template_Cached $cached, Smarty_Internal_Template $_template) {
        $this->_config = $_template->smarty->getTemplateVars('natacache');
 ...
 */
    public function clearAll(Smarty $smarty, $exp_time = null) {
        $this->_config = $smarty->getTemplateVars('natacache');
 ...
 */
    protected function read(array $keys) {
        $data = [];
        foreach ($keys as $key) {
            $k = $this->generateKey($key);
            $data[$key] = Cache::read($k, $this->_config['config']);
        }
 ...
 * @return boolean true on success, false on failure
 */
    protected function write(array $keys) {
        foreach ($keys as $key => $v) {
            $k = $this->generateKey($key);
            Cache::write($k, $v, $this->_config['config']);
        }
 ...
 */
    protected function delete(array $keys) {
        foreach ($keys as $key) {
            $k = $this->generateKey($key);
            Cache::delete($k, $this->_config['config']);
        }
 ...
 *
 * @param string $key key to generate
 * @return string generated key
 */
    protected function generateKey(string $key) {
        $parts = explode('#', $key);
        $tpl = array_pop($parts);
