...
use Nata\Console\Exception\ConsoleException;
use Nata\Console\Exception\MissingOptionException;
 ...
 *
 * `nata my_command --connection default --name=something`
 *
 ...
 *
 * @see \Nata\Console\OptionParser::description()
 * @var string
 */
 ...
 *
 * @see \Nata\Console\OptionParser::epilog()
 * @var string
 */
    protected $_epilog = '';
 ...
 * Option definitions.
 *
 * @see \Nata\Console\OptionParser::addOption()
 * @var array<string, \Nata\Console\InputOption>
 */
    protected $_options = [];
 ...
 *
 * @see \Nata\Console\OptionParser::addArgument()
 * @var array<\Nata\Console\InputArgument>
 */
    protected $_args = [];
 ...
 *
 * @see \Nata\Console\HelpFormatter::setAlias()
 * @var string
 */
    protected $rootName = 'nata';
 ...
 *
 * @param \Nata\Console\OptionParser|array $spec ConsoleOptionParser or spec to merge with.
 * @return $this
 */
    public function merge($spec) {
        if ($spec instanceof OptionParser) {
            $spec = $spec->toArray();
        }
        if (!empty($spec['arguments'])) {
 ...
 * @return $this
 */
    public function setCommand(string $text, bool $normalize = true) {
        $this->_command = $normalize ? Inflector::underscore($text) : $text;
        return $this;
    }
 ...
 *
 * @param \Nata\Console\InputOption|string $name The long name you want to the value to be parsed out
 *   as when options are parsed. Will also accept an instance of InputOption.
 ...
 */
    public function addOption($name, array $options = []) {
        if ($name instanceof InputOption) {
            $option = $name;
            $name = $option->name();
        } else {
            $defaults = [
 ...
 *   An exception will be raised when parse() encounters an invalid value.
 *
 * @param \Nata\Console\InputArgument|string $name The name of the argument.
 *   Will also accept an instance of InputArgument.
 * @param array<string, mixed> $params Parameters for the argument, see above.
 * @return $this
 */
    public function addArgument($name, array $params = []) {
        if ($name instanceof InputArgument) {
            $arg = $name;
 ...
 */
    public function addArguments(array $args) {
        foreach ($args as $name => $params) {
            if ($params instanceof InputArgument) {
                $name = $params;
                $params = [];
            }
 ...
 */
    public function addOptions(array $options) {
        foreach ($options as $name => $params) {
            if ($params instanceof InputOption) {
                $name = $params;
                $params = [];
            }
 ...
 *
 * @return array<\Nata\Console\InputArgument>
 */
 ...
 *
 * @return array<string, \Nata\Console\InputOption>
 */
 ...
 * @throws \Nata\Console\Exception\ConsoleException When an invalid parameter is encountered.
 */
 ...
 */
    public function help(?string $subcommand = null, string $format = 'text', int $width = 72): string {
        ...
        if (isset($this->_subcommands[$subcommand])) {
            ...
            $subparser->setCommand($this->getCommand() . ' ' . $subcommand, false);
 ...
 * @throws \Nata\Console\Exception\ConsoleException When unknown short options are encountered.
 */
 ...
 * @param array<string, mixed> $params The params to append the parsed value into
 * @return array<string, mixed> Params with $option added in.
 * @throws \Nata\Console\Exception\ConsoleException
 */
