 ...
 * Copyright (c) Cake Software Foundation, Inc. (http://natafoundation.org)
 *
 ...
 * - cache: Use the NataPHP caching system as an storage for the session, you will need
 *   to pass the `config` key with the name of an already configured Cache engine.
 * - database: Use the NataPHP ORM to persist and manage sessions. By default this requires
 *   a table in your database named `sessions` or a `model` key in the configuration
 *   to indicate which Table object to use.
 * - nata: Use files for storing the sessions, but let NataPHP manage them and decide
 *   where to store them.
 *
 * The full list of options follows:
 *
 * - defaults: either 'php', 'database', 'cache' or 'nata' as explained above.
 ...
 * @return static
 * @see \Nata\Http\Session::__construct()
 */
 ...
 * - cookieTimeout: The time in minutes the session cookie should be valid for. Maps to the
 *   `session.cookie_lifetime` php.ini config. Defaults to timeout.
 ...
 */
    public function __construct(array $config = []) {
        if (isset($config['timeout'])) {
            $config['ini']['session.gc_maxlifetime'] = $this->_getTimeoutInSeconds($config['timeout']);
        }
        ...
        if (!isset($config['ini']['session.cookie_lifetime']) && (isset($config['cookieTimeout']) || isset($config['timeout']))) {
            $cookieTimeout = (isset($config['cookieTimeout']) ? $config['cookieTimeout'] : $config['timeout']);
            $config['ini']['session.cookie_lifetime'] = $this->_getTimeoutInSeconds($cookieTimeout);
        }

        if (!isset($config['ini']['session.cookie_domain']) && isset($config['cookieDomain'])) {
            $cookieDomain = $config['cookieDomain'];
            $host = $_SERVER['HTTP_HOST'] ?? 'localhost';
            if ($cookieDomain instanceof Closure) {
                $cookieDomain = $cookieDomain($host);
            }
            $config['ini']['session.cookie_domain'] = Text::insert($cookieDomain, [
                'host' => $host
            ], ['before' => '{', 'after' => '}']);
        }
...
/**
 * Get given timeout in seconds.
 * Accepts minutes (int) or strtotime valid string.
 *
 * @param string|int $timeout Timeout in minutes or strtotime string
 * @return int Timeout in seconds
 */
    protected function _getTimeoutInSeconds($timeout) {
        if (is_string($timeout)) {
            return (new Time())->inSeconds($timeout);
        }
        return 60 * $timeout;
    }
 ...
 */
    public function engine($class = null, array $options = []) {
        ...
        return $this->_setEngine($handler);
    }
 ...
 */
    protected function _setEngine(SessionHandlerInterface $handler) {
 ...
 */
    public function read($name = null) {
        ...
        $value = Hash::get($_SESSION, $name);

        if (isset($value['_natasession_timeout_'])) {

            if (!$this->_isTimestampValid($value['_natasession_timeout_'])) {
                $this->delete($name);
                return null;
            }

            $value = $value['_natasession_value_'];
 ...
 *
 * Timeout value must be a unix timestamp or a valid strtotime string.
 *
 * @param string|array $name Name of variable
 * @param mixed $value Value to write
 * @param integer|string $timeout Time out for when the value being written should be deleted
 * @return void
 */
    public function write($name, $value = null, $timeout = null) {
        if (!$this->started()) {
            $this->start();
        }

        if ($timeout !== null) {
            $value = $this->_addTimeout($value, $timeout);
        }
...
/**
 * Add lifetime timestamp to the value being written.
 *
 * @param mixed $value Value to write
 * @param integer|string $timeout Timestamp for when the value
 *  being written should be deem as invalid
 * @return array Value and timeout's timestamp for when it should be invalidated
 */
    protected function _addTimeout($value, $timeout) {
        if (!is_numeric($timeout)) {
            $timeout = (new Time($timeout))->utc();
        }

        return [
            '_natasession_timeout_' => $timeout,
            '_natasession_value_' => $value
        ];

    }

/**
 * Check if given timestamp is in the past.
 *
 * @param integer $timeout Timeout UTC unix timestamp
 * @return bool True if timestamp is still in the future
 */
    protected function _isTimestampValid($timeout) {
        return $timeout > (new Time)->utc();
    }
