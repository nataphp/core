...
namespace Nata\Core\Configure\Engine;

use Nata\Core\App;
use Nata\Core\Configure\Engine;
use Exception;
 ...
 * ```
 *
 * @see Nata\Core\Configure::load() for how to load custom configuration files.
 */
class PhpConfig extends Engine {
 ...
 */
    public function dump($key, array $data) {
        $contents = '<?php' . "\n" . 'return ' . var_export($this->_check($data), true) . ';';
        $filename = $this->_getFilePath($key);
        return file_put_contents($filename, $contents) > 0;
    }

/**
 * Checks the data if there are any objects in it that don't
 * have __set_state implemented.
 *
 * @param array $data Data to dump.
 * @return array Data with objects removed.
 */
