 ...
 * - prefix = string prefix for filename, default => nata_
 ...
 */
    public function init($settings = []) {
        ...
        if (DS === '\\') {
            $this->_settings['isWindows'] = true;
        }
        if (substr($this->_settings['path'], -1) !== DS) {
            $this->_settings['path'] .= DS;
        }
 ...
 */
    public function write($key, $data, $duration) {
        ...
        $lineBreak = "\n";
        if ($this->_settings['isWindows']) {
            $lineBreak = "\r\n";
        }

        if (!empty($this->_settings['serialize'])) {
            if ($this->_settings['isWindows']) {
 ...
 */
    public function read($key) {
        ...
        if ($cachetime !== false && ($cachetime < $time || ($time + $this->_settings['duration']) < $cachetime)) {
            if ($this->_settings['lock']) {
                $this->_File->flock(LOCK_UN);
            }
            return null;
        ...
        if ($data !== '' && !empty($this->_settings['serialize'])) {
            if ($this->_settings['isWindows']) {
 ...
 */
    public function clear($check) {
        ...
        $dir = dir($this->_settings['path']);

        if ($check) {
            $now = time();
            $threshold = $now - $this->_settings['duration'];
        }

        $prefixLength = strlen($this->_settings['prefix']);

        while (($entry = $dir->read()) !== false) {
            if (substr($entry, 0, $prefixLength) !== $this->_settings['prefix']) {
                continue;
            }
 ...
 */
    protected function _setKey($key, $createKey = false) {
        ...
        $dir = $this->_settings['path'] . $groups;
        ...
        if (empty($this->_File) || $this->_File->getBaseName() !== $key) {
            ...
            if (!$exists && !chmod($this->_File->getPathname(), (int)$this->_settings['mask'])) {
 ...
 */
    protected function _active() {
        $dir = new SplFileInfo($this->_settings['path']);
        if ($this->_init && !($dir->isDir() && $dir->isWritable())) {
            $this->_init = false;
            trigger_error(sprintf('%s is not writable', $this->_settings['path']), E_USER_WARNING);
            return false;
        }
        return true;
    }
 ...
 **/
    public function clearGroup($group) {
        $directoryIterator = new RecursiveDirectoryIterator($this->_settings['path']);
