 ...
 */
class Memcached extends Engine {
 ...
 */
	protected $_serializers = array(
		'igbinary' => MemcachedLib::SERIALIZER_IGBINARY,
		'json' => MemcachedLib::SERIALIZER_JSON,
		'php' => MemcachedLib::SERIALIZER_PHP
	);
 ...
 */
	public function init($settings = array()) {
		...
		if (!is_array($this->_settings['servers'])) {
			$this->_settings['servers'] = array($this->_settings['servers']);
		}
		...
		$this->_Memcached = new MemcachedLib($this->_settings['persistent'] ? (string)$this->_settings['persistent'] : null);
		$this->_setOptions();
		...
		if ($this->_settings['login'] !== null && $this->_settings['password'] !== null) {
			if (!method_exists($this->_Memcached, 'setSaslAuthData')) {
				throw new CacheException(
					__('Memcached extension is not build with SASL support')
				);
			}
			$this->_Memcached->setSaslAuthData($this->_settings['login'], $this->_settings['password']);
		}
 ...
 */
	protected function _setOptions() {
		$this->_Memcached->setOption(MemcachedLib::OPT_LIBKETAMA_COMPATIBLE, true);

		$serializer = strtolower($this->_settings['serialize']);
		if (!isset($this->_serializers[$serializer])) {
			throw new CacheException(
				__('%s is not a valid serializer engine for Memcached', $serializer)
			);
		}

		if ($serializer !== 'php' && !constant('\MemcachedLib::HAVE_' . strtoupper($serializer))) {
			throw new CacheException(
				__('Memcached extension is not compiled with %s support', $serializer)
			);
		}

		$this->_Memcached->setOption(MemcachedLib::OPT_SERIALIZER, $this->_serializers[$serializer]);

		// Check for Amazon ElastiCache instance
		if (defined('\MemcachedLib::OPT_CLIENT_MODE') && defined('\MemcachedLib::DYNAMIC_CLIENT_MODE')) {
			$this->_Memcached->setOption(MemcachedLib::OPT_CLIENT_MODE, MemcachedLib::DYNAMIC_CLIENT_MODE);
		}

		$this->_Memcached->setOption(MemcachedLib::OPT_COMPRESSION, (bool)$this->_settings['compress']);
	}
 ...
 */
    public function check($key) {
        return !empty($this->_Memcached->get($key));
    }
 ...
 */
	public function clearGroup($group) {
		return (bool)$this->_Memcached->increment($this->_settings['prefix'] . $group);
	}
