 ...
 */
    public function read($key) {
        if (xcache_isset($key)) {
            ...
            if ($cachetime < $time || ($time + $this->_settings['duration']) < $cachetime) {
                return null;
            }
 ...
 **/
    public function groups() {
        ...
        foreach ($this->_settings['groups'] as $group) {
            $value = xcache_get($this->_settings['prefix'] . $group);
            if (!$value) {
                $value = 1;
                xcache_set($this->_settings['prefix'] . $group, $value, 0);
            }
            $result[] = $group . $value;
        }
        return $result;
 ...
 **/
    public function clearGroup($group) {
        return (bool)xcache_inc($this->_settings['prefix'] . $group, 1);
    }
    ...
    protected function _auth($reverse = false) {
        ...
        foreach ($keys as $key => $setting) {
            if ($reverse) {
                ...
                if (!empty($this->_settings[$setting])) {
                    $_SERVER[$key] = $this->_settings[$setting];
                } elseif (!empty($this->_settings[$key])) {
                    $_SERVER[$key] = $this->_settings[$key];
                } else {
                    $_SERVER[$key] = $value;
                }
            }
