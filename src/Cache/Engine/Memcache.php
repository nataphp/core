 ...
 */
    public function init($settings = array()) {
        ...
        if ($this->_settings['compress']) {
            $this->_settings['compress'] = MEMCACHE_COMPRESSED;
        }
        ...
        if (!isset($this->_Memcache)) {
            ...
            foreach ($this->_settings['servers'] as $server) {
                ...
                if ($this->_Memcache->addServer($host, $port, $this->_settings['persistent'])) {
                    $return = true;
                }
            }
 ...
 */
    public function write($key, $value, $duration) {
        ...
        return $this->_Memcache->set($key, $value, $this->_settings['compress'], $duration);
    }
 ...
 */
    public function check($key) {
        return !empty($this->_Memcache->get($key));
    }
    ...
    public function increment($key, $offset = 1) {
        if ($this->_settings['compress']) {
            throw new \CacheException(
                sprintf('Method increment() not implemented for compressed cache in %s', __CLASS__)
            );
        }
 ...
 */
    public function decrement($key, $offset = 1) {
        if ($this->_settings['compress']) {
            throw new \CacheException(
                sprintf('Method decrement() not implemented for compressed cache in %s', __CLASS__)
            );
        }
 ...
 **/
    public function clearGroup($group) {
        return (bool)$this->_Memcache->increment($this->_settings['prefix'] . $group);
    }
}
