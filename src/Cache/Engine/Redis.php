    ...
    protected function _connect() {
        ...
        try {
            ...
            if (empty($this->_settings['persistent'])) {
                $return = $this->_Redis->connect($this->_settings['server'], $this->_settings['port'], $this->_settings['timeout']);
            } else {
                $return = $this->_Redis->pconnect($this->_settings['server'], $this->_settings['port'], $this->_settings['timeout']);
            }
 ...
 */
    public function clear($check) {
        ...
        $keys = $this->_Redis->getKeys($this->_settings['prefix'] . '*');
 ...
 **/
    public function groups() {
        ...
        foreach ($this->_settings['groups'] as $group) {
            $value = $this->_Redis->get($this->_settings['prefix'] . $group);
            if (!$value) {
                $value = 1;
                $this->_Redis->set($this->_settings['prefix'] . $group, $value);
            }
            $result[] = $group . $value;
        }
        return $result;
 ...
 **/
    public function clearGroup($group) {
        return (bool)$this->_Redis->incr($this->_settings['prefix'] . $group);
    }
