 ...
 * }}}
 *
 * Or engine by preference/availability order
 *
 * {{{
 *    Cache::config('shared', array(
 *        'engine' => array('Apc', 'File'),
 *        'prefix' => 'my_app_'
 *  ));
 * }}}
 *
 ...
 * - `Apc` - Uses the APC object cache, one of the fastest caching engines.
 * - `Memcache` - Uses the PECL::Memcache extension and Memcached for storage.
 *      Fast reads/writes, and benefits from memcache being distributed.
 * - `Xcache` - Uses the Xcache extension, an alternative to APC.
 * - `Wincache` - Uses Windows Cache Extension for PHP. Supports wincache 1.1.0 and higher.
 *
 ...
 *
 * @see config/core.inc.php for configuration settings
 * @param string $name Name of the configuration
 * @param array $settings Optional associative array of settings passed to the engine
 * @return array(engine, settings) on success, false on failure
 * @throws Exception
 */
    public static function config($name, array $settings = []) {
        $current = [];
        ...
        if (empty($current['engine'])) {
            return false;
        }

        $engine = $current['engine'];
        if (isset(static::$_engines[$name]) && !$current = static::set($current, null, $name)) {
            return false;
        }

        static::$_config[$name] = $current;

        return compact('engine', 'current');
    }

/**
 * Initializes cache configuration if not yet initialized.
 *
 ...
 */
    private static function _init($config) {
        ...
        return static::_buildEngine($config);
    }

/**
 * Finds and builds the instance of the required engine(s) class(es).
 *
 * @param string $name Name of the config array that needs an engine instance built
 * @return boolean
 * @throws Exception
 */
    protected static function _buildEngine($name) {
        ...
        $engines = (array)$config['engine'];

        foreach ($engines as $engine) {
            list($plugin, $class) = pluginSplit($engine, true);
            $class = Inflector::camelize($class);
            $className = App::className($plugin . $class, 'Cache/Engine');

            /**
             * This will ensure the cache engines are not dependent of autoloader,
             * allowing to be used early in the initialization chain
             */
            if (!$className) {
                return false;
            }

            if (!is_subclass_of($className, '\Nata\Cache\Engine')) {
                throw new Exception('Cache engines must use \Nata\Cache\Engine as a base class.');
            }

            $engine = new $className();
            if ($engine->init($config) === false) {
                continue;
            }

            static::$_engines[$name] = $engine;
            $settings = static::$_engines[$name]->settings();
            $settings['engine'] = $className;
            if ($settings['probability'] && time() % $settings['probability'] === 0) {
                static::$_engines[$name]->gc();
            }

            static::$_config[$name] = static::$_engines[$name]->settings($settings);
...
/**
 * Returns an array containing the initialized Cache engines.
 *
 * @return array Array of initialized engines Cache config names.
 */
        ...
        if (static::$_reset === true) {
            ...
            if (empty($settings)) {
                ...
                if (isset($settings['duration']) && !is_numeric($settings['duration'])) {
                    $settings['duration'] = (new Time($settings['duration']))->inSeconds();
                }
            }
 ...
 * @return void
 */
    public static function gc(string $config = 'default', $expires = null) {
        if (!static::_init($config)) {
            return false;
        }
 ...
 */
    public static function read(string $key, string $config = 'default') {
        ...
        if ($key === 'mailbox_customlabels_4') {
            return null;
            print_a(static::$_engines[$config]->read($settings['prefix'] . $key));
            die;
        }
 ...
 */
    public static function remember(string $key, callable $callable, $config = 'default') {
        $value = static::read($key, $config);
        if ($value === null) {
            $value = $callable();
            if ($value !== null) {
                static::write($key, $value, $config);
            }
        }
        return $value;
    }
...
/**
 * Check if key exists. Will automatically use the currently
 * active cache configuration. To set the currently active configuration use
 * Cache::config().
 *
 * @param string $key Identifier for the data
 ...
 */
    public static function check($key, $config = 'default') {
        $settings = static::settings($config);
        if (!$settings) {
            return false;
        }
        ...
        return static::$_engines[$config]->check($settings['prefix'] . $key);
    }

/**
 * Consume the key's data.
 * Returns and deletes the value from cache.
 *
 * @param string $key Identifier for the data
 * @param string $config optional name of the configuration to use. Defaults to 'default'
 * @return mixed The cached data, or null if the data doesn't exist, has expired, or if there was an error fetching it
 */
    public static function consume($key, $config = 'default') {
        $data = static::read($key, $config);
        static::delete($key, $config);
        return $data;
    }
 ...
 * @return mixed The cached data, or false if the data doesn't exist, has expired, or if there was an error fetching it
 */
    public static function info(string $key, $config = 'default') {
        $settings = static::settings($config);
        if (empty($settings)) {
            return false;
        }
        ...
        return static::$_engines[$config]->info($settings['prefix'] . $key);
    }
 ...
 * @return array If successfull initialization, list of settings for this engine
 *  empty array otherwise
 * @see Cache::config()
 */
    public static function settings($name = 'default'): ?array {
        if (!static::_init($name)) {
            return null;
        }
