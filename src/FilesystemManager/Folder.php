 ...
 */
    public function name() {
        $parts = explode(DS, rtrim($this->path, DS));
        return end($parts);
    }
 ...
 */
    public function find($regexpPattern = '.*', $sort = false, $fullPath = false) {
        [, $files] = $this->read($sort, false, $fullPath);
 ...
 */
    public function findRecursive($pattern = '.*', $sort = false, $fullPath = false) {
        if (!$this->pwd()) {
            return [];
        }

        $startsOn = $this->path;
        $out = $this->_findRecursive($pattern, $sort, $fullPath = false);
        $this->cd($startsOn);
 ...
 * @return array Files matching pattern
 */
    protected function _findRecursive($pattern, $sort = false, $fullPath = false) {
        $found = [];

        [$dirs, $files] = $this->read($sort, false, $fullPath);

        foreach ($files as $file) {
            if (($pattern === '.*' && strpos($file, '.') !== false) || preg_match('/^' . $pattern . '$/i', $file)) {
    ...
    public function tree($path = null, $exceptions = false, $type = null) {
        ...
        foreach ($iterator as $itemPath => $fsIterator) {
            if ($skipHidden) {
                ...
                if (substr($subPathName, 0, 1) === '.' || strpos($subPathName, DS . '.') !== false) {
                    continue;
                }
            }
 ...
 * Returns the size in bytes of this Folder and its contents.
 *
 * @deprecated Use Folder::size() instead
 * @return integer size in bytes of current folder
 */
 ...
 *
 * @return integer Timestamp of last modification, or false in case of an error
 */
    public function lastChange(): ?int {
        return $this->_getStats('mtime');
    }

/**
 * Returns last access time.
 *
 * @return integer Timestamp of last access, or false in case of an error
 */
    public function lastAccess(): ?int {
        return $this->_getStats('atime');
    }
 ...
 */
    protected function _getStats($name) {
        $stat = stat($this->pwd());
        if (!$stat) {
            return null;
        }
        return isset($stat[$name]) ? $stat[$name] : null;
    }
 ...
 */
    public function inNataPath($path = '') {
 ...
 */
    public function realpath($path) {
        ...
        return Folder::slashTerm(realpath($path));
    }
