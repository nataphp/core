 ...
 * @var array
 */
    protected $_defaultSizes = [64, 512, 1024, 1920, 'original'];
 ...
 */
    public function get(string $aspectRatio = 'original', int|string $width = null, int $dpi = 1, string $store = null, string $variant = null): array {
        ...
        if ($width !== null) {
            ...
            return $this->_getWidth($aspectRatio, $width * $dpi, false, $store, $variant);
        }
 ...
 */
    public function getSrcset($aspectRatio = 'original', string $store = null, string $variant = null): string {
        ...
        foreach ($this->_get($aspectRatio, $store, $variant) as $width => $img) {
            foreach ($dpis as $dpi) {
                if ($srcset) {
                    $srcset .= ', ';
                }
 ...
 * @return void
 */
    protected function __mapAspectRatioSizes(string $aspectRatio, array $sizes, string $variant) {
        ...
        foreach ($sizes as $width) {
            if ($width === 'original') {
                $width = $originalWidth;
                // Set original
                if ($isOriginalAspectRatio === true && !isset(static::$_set[$this->_sha1][$aspectRatio][$variant][$width])) {
                    $this->_addSizeToSet(
                        $aspectRatio,
                        FileStorage::url($this->_image),
                        $originalWidth,
