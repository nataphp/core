...
namespace Nata\FilesystemManager\File\Image\Processor\GD;
 ...
 * Constructor.
 *
 * @param GdImage|resource $gdResource GDimage resource
 * @param array $options Options
 * @return void
 */
	public function __construct($gdResource, array $options = []) {
		$this->_gdImage = $gdResource;
 ...
 */
	public function canvasImageScale() {
		$imageOriginalWidth = imagesx($this->_gdImage);
		$imageOriginalHeight = imagesy($this->_gdImage);
		...
		if ($this->_config['prescale'] !== false) {
			$this->_preScale = 1 / $scale / $this->_config['minScale'];

			if ($this->_preScale < 1) {
				$this->canvasImageResample(
					ceil($imageOriginalWidth * $this->_preScale),
					ceil($imageOriginalHeight * $this->_preScale)
				);

				$this->_config['cropWidth'] = ceil($this->_config['cropWidth'] * $this->_preScale);
				$this->_config['cropHeight'] = ceil($this->_config['cropHeight'] * $this->_preScale);
			} else {
				$this->_preScale = 1;
			}
 ...
 */
	public function canvasImageResample($width, $height) {
		...
		imagecopyresampled(
			...
			$width,
			$height,
			imagesx($this->_gdImage),
			imagesy($this->_gdImage)
		);

		$this->_gdImage = $oCanvas;
 ...
 */
	public function analyse() {
		...
		$w = $this->_w = imagesx($this->_gdImage);
		$h = $this->_h = imagesy($this->_gdImage);

		$this->_outputData = new SplFixedArray($h * $w * 4);
		$this->_aSample = new SplFixedArray($h * $w);
		...
		for ($y = 0; $y < $h; $y++) {
			for ($x = 0; $x < $w; $x ++) {
				...
				$this->_outputData[$p + 1] = $this->edgeDetect($x, $y, $w, $h, $p);

				$this->_outputData[$p] = $this->skinDetect(
					$aRgb[0],
					$aRgb[1],
					$aRgb[2],
					$this->sample($x, $y)
				...
				$this->_outputData[$p + 2] = $this->saturationDetect(
					$aRgb[0],
					$aRgb[1],
					$aRgb[2],
					$this->sample($x, $y)
 ...
 */
	protected function downSample($factor) {
		$width = floor($this->_w / $factor);
		$height = floor($this->_h / $factor);
		...
		for ($y = 0; $y < $height; $y++) {
			for ($x = 0; $x < $width; $x++) {
				...
				for ($v = 0; $v < $factor; $v++) {
					for ($u = 0; $u < $factor; $u++) {
						$p = (($y * $factor + $v) * $this->_w + ($x * $factor + $u)) * 4;

						$pR = $this->_outputData[$p];
						$pG = $this->_outputData[$p + 1];
						$pB = $this->_outputData[$p + 2];
						$pA = $this->_outputData[$p + 3];
						$r += $pR;
						$g += $pG;
						$b += $pB;
						$a += $pA;
 ...
 */
	protected function generateCrops() {
		$w = imagesx($this->_gdImage);
		$h = imagesy($this->_gdImage);
 ...
 */
	protected function score($output, $crop) {
		...
		$outputHeightDownSample = floor($this->_h / $downSample) * $downSample;
		$outputWidthDownSample = floor($this->_w / $downSample) * $downSample;
		$outputWidth = floor($this->_w / $downSample);
 ...
 */
	protected function sample($x, $y, $p = null) {
		...
		if (isset($this->_aSample[$p])) {
			return $this->_aSample[$p];
		}

		$aRgbColor = $this->getRgbColorAt($x, $y);
		$this->_aSample[$p] = $this->cie($aRgbColor[0], $aRgbColor[1], $aRgbColor[2]);
		return $this->_aSample[$p];
	}
